app = angular.module('closing-checklist-directive', ['closingChecklistService'])
app.directive "closingChecklist", ->
  restrict: "E"
  scope:
    loan_number: "=loanNumber"

  templateUrl: "closing_checklist.html"
  controller: ClosingChecklistDirectiveCtrl
  controllerAs: 'ctrl'
  
class ClosingChecklistDirectiveCtrl
  ...
  formatThreePercentages: ->
    threeQuestions = _.select @questions, (q) -> q.input_type == 'three_percentages'
    _.each threeQuestions, (q) ->
      q.percentages = ['', '', '']
      return unless q.answer
      matches = q.answer.match three_pct_regex
      q.percentages = _.map matches.slice(1,4), (pct) ->
        f = parseFloat(pct)
        if f then f.toPrecision(4) else ''
...
ClosingChecklistDirectiveCtrl.$inject = [ "$rootScope", "$scope", "$http", 'ClosingChecklistService' ]
window.ClosingChecklistDirectiveCtrl = ClosingChecklistDirectiveCtrl
